...
from ui_components import HERO_ORB_CFG
...
class LevelEditor:
    ...
    def _load_all_bg_sets(self):
        ...
        _CFG = {
            "forest": (
                os.path.join("assets", "Platform tiles", "Background", "Forest"),
                [("Layer_03.png", 0.05), ("Layer_02.png", 0.20), ("Layer_01.png", 0.45)],
                (135, 206, 235),
            ),
            "dungeon": (
                os.path.join("assets", "Platform tiles", "Background", "Dungeon"),
    ...
    def _build_previews(self):
        ...
        # Tile previews — build one set per theme so switching bg_set updates previews
        self._tile_previews_by_theme = {}
        for theme in ("forest", "dungeon"):
            ttm = TileTextureManager.get(theme)
            previews = {}
            _tile_src = {
                T_GROUND:   lambda t=ttm: t.ground_tex(False),
                T_PLATFORM: lambda t=ttm: t.platform_tex(),
                T_COIN:     lambda t=ttm: t.anim_frame("coin"),
                T_SPIKE:    lambda t=ttm: t.anim_frame("spike"),
                T_TORCH:    lambda t=ttm: t.anim_frame("torch"),
            }
            for tid, src_fn in _tile_src.items():
                ...
                previews[tid] = surf
            self._tile_previews_by_theme[theme] = previews
        # _tile_previews kept as alias to forest for any legacy code paths
        self._tile_previews = self._tile_previews_by_theme["forest"]
    ...
    def _draw_canvas(self):
        ...
        self.level.tilemap.draw(surf, (int(self.cam_x), int(self.cam_y)),
                                getattr(self.level, "bg_set", "forest"))
    ...
    def _content_tiles(self, surf, sy):
        ...
        cw  = (PANEL_W - 14) // 2
        y0  = sy + 4
        theme_previews = self._tile_previews_by_theme.get(
            getattr(self.level, "bg_set", "forest"),
            self._tile_previews_by_theme["forest"]
        )
