...
# Forest tileset — 128×96 px, 16×16 tiles (8 cols × 6 rows)
_TILESET_PATH = os.path.join("assets", "Platform tiles", "Tileset", "Forest", "Tileset.png")
_ROW_GRASS_TOP  = 0;  _COL_GRASS_TOP  = 0
_ROW_DIRT_FILL  = 1;  _COL_DIRT_FILL  = 0
_PLAT_LIGHT     = [(0, 0), (1, 1), (2, 1)]

# Dungeon tileset — metroidvania pack, 16×16 tiles
_DUNGEON_TILESET_PATH = os.path.join("assets", "Platform tiles", "Tileset", "Dungeon", "tileset.png")
_ROW_DUNG_TOP  = 0;  _COL_DUNG_TOP  = 1   # stone surface (exposed top)
_ROW_DUNG_FILL = 1;  _COL_DUNG_FILL = 1   # stone interior (buried fill)
_PLAT_DUNG     = [(0, 1), (0, 1), (1, 1)] # dungeon platform composite


class TileTextureManager:
    """
    Loads platform tile textures for a given theme ("forest" or "dungeon").
    One instance per theme — call TileTextureManager.get(theme) everywhere.
    Lazy-initialised on first use so pygame must be ready first.
    """
    _instances: dict = {}

    @classmethod
    def get(cls, theme: str = "forest"):
        if theme not in cls._instances:
            inst = TileTextureManager()
            inst._load(theme)
            cls._instances[theme] = inst
    ...
    def _load(self, theme: str = "forest"):
        if theme == "dungeon" and os.path.exists(_DUNGEON_TILESET_PATH):
            self._ts = pygame.image.load(_DUNGEON_TILESET_PATH).convert_alpha()
            self._grass_top = self._tile(_ROW_DUNG_TOP,  _COL_DUNG_TOP)
            self._dirt_fill = self._tile(_ROW_DUNG_FILL, _COL_DUNG_FILL)
            self._platform  = self._make_platform_tex(_PLAT_DUNG)
        else:
            ...
            self._platform  = self._make_platform_tex(_PLAT_LIGHT)

        # Animated tiles loaded from strip files in miscellaneous sprites
    ...
    def _make_platform_tex(self, tiles=None):
        if tiles is None:
            tiles = _PLAT_LIGHT
        srcs = [self._tile(r, c) for r, c in tiles]
        if not srcs[0]:
            return None
...
class TileMap:
    ...
    def draw(self, surface, camera_offset, theme: str = "forest"):
        ox, oy = camera_offset
        ttm = TileTextureManager.get(theme)
