...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, save_data):
        super().__init__()
        starting_wid     = save_data.get("unlocked_weapons", [W_SWORD])[0]
        self.MAX_HP      = character_data.get(starting_wid, "max_hp", 100)
        self._cooldowns  = {
            wid: character_data.get(wid, "shoot_cooldown", 30)
    ...
    def _load_char_anim(self, wid):
        ...
        """
        cfg      = character_data.CHARS.get(wid, {})
    ...
    def try_shoot(self, pressed):
        ...
        self.shoot_timer    = self._cooldowns.get(w, 30)
        self._just_attacked = 10
        cx, cy = self.rect.centerx, self.rect.centery
        cd = character_data.CHARS.get(w, {})
        if w == W_SWORD:
            p = SwordSlash(self.rect, self.facing,
                           damage=cd.get("attack_damage"))
        elif w == W_BOW:
            p = Arrow(cx + self.facing * 20, cy, self.facing,
                      damage=cd.get("attack_damage"),
                      speed=cd.get("arrow_speed"),
                      lifetime=cd.get("arrow_lifetime"))
        elif w == W_STAFF:
            p = MagicBolt(self.rect, self.facing,
                          damage=cd.get("attack_damage"),
                          duration=cd.get("blast_duration"))
        else:
            return None
